...
from main_model_15 import HiFuse_Base as create_model
...
def main(args):
    ...
    if args.RESUME:
        path_checkpoint = "./model_weight/checkpoint/ckpt_best_300.pth"
    ...
    train_losses,train_accs,va_losses,val_accs=[],[],[],[]
    epoch_list=[]
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        if best_acc < val_acc:
            ...
            torch.save(model.state_dict(), "./model_weight/CT_maxvit.pth")
        ...
        if epoch ==args.eppochs:
            plt.figure(figsize=(10,5))
            plt.figure(figsize==(8,6))
            plt.plot(epochs_list,train_losses,label='Train Loss')
            plt.plot(epoch_list,train_accs,label='Train Accuracy')
            plt.xticks(np.arange(0,args.epochs+1,20))
            plt.xlabel('Epoch')
            plt,ylabel('value')
            plt.title('Train Loss and Accuracy over 100 epochs')
            plt,legend()
            plt.grid(True)
            plt.show()

            # Validation Metrics 图
            plt.figure(figsize=(10, 5))
