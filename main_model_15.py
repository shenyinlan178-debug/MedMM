...
class main_model_11(nn.Module):
    ...
    def __init__(self, num_classes, patch_size=4, in_chans=3, embed_dim=96, depths=(2, 2, 2, 2),
        ...
        self.hmsf1 = HMSF1(96)
        self.hmsf2 = HMSF1(192)
        self.hmsf3 = HMSF1(384)
        self.hmsf4 = HMSF1(768)

        self.fsm1 = FSM(96, 96)
        self.fsm2 = FSM(192, 192)
        self.fsm3 = FSM(384, 384)
        self.fsm4 = FSM(768, 768)

        ###### Hierachical Feature Fusion Block Setting #######

        self.fu1 = cofusion_block(ch_1=96, ch_2=96, channels=96,r_2=16, ch_int=96, ch_out=96, drop_rate=HFF_dp)
        self.fu2 = cofusion_block(ch_1=192, ch_2=192,channels=192, r_2=16, ch_int=192, ch_out=192, drop_rate=HFF_dp)
        self.fu3 = cofusion_block(ch_1=384, ch_2=384,channels=384, r_2=16, ch_int=384, ch_out=384, drop_rate=HFF_dp)
        self.fu4 = cofusion_block(ch_1=768, ch_2=768, channels=768,r_2=16, ch_int=768, ch_out=768, drop_rate=HFF_dp)
    ...
    def forward(self, imgs,mask_ratio=0.6):
        ...
        # FOM
        x1_fsm = self.fsm1(x_s_1)
        x2_fsm = self.fsm2(x_s_2)
        x3_fsm = self.fsm3(x_s_3)
        x4_fsm = self.fsm4(x_s_4)

        coarse_att = self.UNM(x4_fsm, x3_fsm, x2_fsm, x1_fsm)
...
class LFEConv(nn.Module):
    def __init__(self, dim):
        super(LFEConv, self).__init__()
...
class cofusion_block(nn.Module):
...
class HMSF1(nn.Module):
    def __init__(self, channel):
        super(HMSF1, self).__init__()
        t = int(abs((log(channel, 2) + 1) / 2))
        k = t if t % 2 else t + 1
        self.conv2d = ConvBNR(channel, channel, 3)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
...
def MedMM_Tiny(num_classes: int):
...
def MedMM_Small(num_classes: int):
    model = main_model_11(depths=(2, 2, 6, 2),
                     conv_depths=(2, 2, 6, 2),
                     num_classes=num_classes)
    return model

def MedMM_Base(num_classes: int):
